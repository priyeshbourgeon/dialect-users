                            ...
                            <div class="uk-margin-medium-bottom">
                                    ...
                                    <div class=" form_group">
                                        <div class=" form_group">
                                            ...
                                            <div class="uk-form-controls">
                                                <input id="search-category" class="uk-input" type="text"
                                                    placeholder="Search">
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="form_group" style="margin-top:10px;">
                                    <ul class="sector_list">
                                        <li>
                                            <div class="s_block active" onClick="window.location.reload();">All</div>
                                        </li>
                                        @foreach(range('A', 'Z') as $char)
                                        <li>
                                            <div class="s_block alpha-category" data-alpha="{{ $char }}">{{ $char }}
                                            </div>
                                        </li>
                                        @endforeach
                                    </ul>
                            ...
                            <div uk-grid>
                                <div class="uk-width-expand@m">
                                    <ul class="list_catg uk-margin-medium-bottom category_list">
                                        ...
                                        <li><a class="category" data-id="{{ $cat->id }}"
                                                data-name="{{ $cat->name }}">{{ $cat->name ?? '' }}</a></li>
                                        @endforeach
                                    </ul>
                                </div>
        ...
        if (keyword) {
            $.ajax({
                ...
                beforeSend: function() {
                    ...
                    $('#cat_selected').text('');
                    $(".category_list").html(
                        '<li class="uk-text-large uk-text-danger uk-text-center"><div uk-spinner="ratio: 3"></div></li>'
                        );
                },
                    ...
                    if (res) {
                        ...
                        $.each(res, function(key, value) {
                            $(".category_list").append(
                                '<li style="background:#eeeeee"><a class="subcategory" data-id="' +
                                res[key].id + '" data-name="' + res[key].name +
                                '">' + res[key].name + '</a></li>');
                        });
                    }
                }
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res) {
                        ...
                        $.each(res, function(key, value) {
                            $(".category_list").append(
                                '<li><a class="category" data-id="' + res[key]
                                .id + '" data-name="' + res[key].name + '">' +
                                res[key].name + '</a></li>');
                        });
                    }
                }
                ...
                success: function(res) {
                    ...
                    if (res) {
                        $.each(res, function(key, value) {
                            ...
                            $("#parentbox").append(
                                '<li style="width: 100%"><a class="subcategory" data-id="' +
                                res[key].id + '">' + res[key].name + '</a></li>'
                                );
                        });
                    } else {
                        $('#parentbox').append(
                            '<p class="uk-text-large uk-text-danger uk-text-center">No Data Found</p>'
                            );
                    }
                }
            });
